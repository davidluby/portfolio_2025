...
const Sim = () => {
  ...
  useEffect (() => {
    ...
    let mesh_vertices = []
    mesh_vertices = new Float32Array(create_mesh(mesh_vertices, element))

    let out = []
    let color_data = new Float32Array(flu.x_dim * flu.y_dim * 4 * 6)
    ...
    function create_mesh (mesh_vertices, element) {
      ...
      for (let x = 0; x < flu.x_dim - 0; x++) {
        for (let y = 0; y < flu.y_dim - 0; y++) {
          for (let i = 0; i < 6; i++) {
            idx = i * 3
            let translated_vertex = [
              element[idx]    + 2 * x * h - offset_x,
              element[idx + 1] + 2 * y * h -offset_y,
              0
            ]
            mesh_vertices.push(...translated_vertex)
          }
        }
      }
    ...
    function gl_color (dta) {
      for (let i = 0; i < flu.d.length; i++) {
        let element_idx = 24 * i
        let val = flu.d[i]
        for (let j = 0; j < 6; j++) {
          let vertex_idx = 4 * j
          dta[element_idx + vertex_idx] = val * 21 / 255
          dta[element_idx + vertex_idx + 1] = val * 76 / 255
          dta[element_idx + vertex_idx + 2] = val * 121 / 255
          dta[element_idx + vertex_idx + 3] = 1
        }
      }

      /* for (let i = 0; i < flu.x_dim; i++) {
        for (let j = 0; j < flu.y_dim; j++) {
          let idx = j * flu.y_dim + i
          let val = flu.d[idx]
          let element_idx = idx * 24
          for (let k = 0; k < 6; k++) {
            let vertex_idx = k * 4
            dta[element_idx + vertex_idx] = val * 21 / 255
            dta[element_idx + vertex_idx + 1] = val * 76 / 255
    ...
    function update () {
      if (scene.frame_nr % 100 == 0) {
        const x = 0.5 + Math.cos(Math.random() * Math.PI) / 4
        const y = 0.5 + Math.cos(Math.random() * Math.PI) / 4
        interact(x, y, false)
      }
      /* if (scene.frame_nr < 10) (
  ...
  return (
    <canvas id='fluid' height={1000} width={1900} className='absolute z-[-1] left-0 top-0 w-full h-full'></canvas>
  )
}
